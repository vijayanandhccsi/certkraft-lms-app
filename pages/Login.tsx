...
import { Lock, Mail, ArrowRight, AlertCircle, Linkedin, Loader2 } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Here we derive a display name from the email for demo purposes.
  ...
  const handleSocialLogin = async (provider: string) => {
    setLoading(true);
    try {
      await login(`New ${provider} User`, `user@${provider.toLowerCase()}.com`);
      navigate('/student/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center px-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-slate-400 text-xs mt-8">
          &copy; {new Date().getFullYear()} Certkraft Inc.
        </p>
      </div>
    </div>
  );
