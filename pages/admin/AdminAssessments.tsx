...
import { useAssessments, Assessment, Question, Option, QuestionType } from '../../contexts/AssessmentContext';
...
const AdminAssessments: React.FC = () => {
  ...
  const handleAiGenerateQuestions = async () => {
    ...
    try {
      const results: GeneratedQuestion[] = await generateQuizQuestions(currentAssessment.title, currentAssessment.description);
      
      if (results && results.length > 0) {
        const newQuestions: Question[] = results.map((q, idx) => ({
          ...
          options: q.options.map((opt, oIdx) => ({
            id: `ai_opt_${Date.now()}_${idx}_${oIdx}`,
            text: opt.text,
        ...
        alert("AI could not generate questions. Please ensure the title and description are clear.");
      }
    } catch (e) {
      console.error(e);
